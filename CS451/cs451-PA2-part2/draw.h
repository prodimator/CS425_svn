...
void Motion(int x, int y)
{
	...
	for (int i = 0; i < m.v_size;i++){
		...
		float sumy = 0;
		float sumz = 0;
		for (int j = 0; j < FFD_lattice.size(); j++){
			sumx += FFD_param[i][j] * FFD_lattice[j][0];
			sumy += FFD_param[i][j] * FFD_lattice[j][1];
			sumz += FFD_param[i][j] * FFD_lattice[j][2];
		}
		m.vertices[i].p[0] = sumx;
		m.vertices[i].p[1] = sumy;
		m.vertices[i].p[2] = sumz;
