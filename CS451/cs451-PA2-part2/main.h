...
void parameterizeModel()
{
	...
	FFD_param = new double *[m.v_size];
	for (int i = 0; i < m.v_size; i++){
		FFD_param[i] = new double[lattice_nx*lattice_ny*lattice_nz];
	}

	for (unsigned int a = 0; a < m.v_size; a++)
	{
		vertex & v = m.vertices[a];
		//cout << lattice_nx << ", " << lattice_ny << ", " << lattice_nz << "\n";
		//cout << m.vertices[a].p[0] << " " << m.vertices[a].p[1] << " " << m.vertices[a].p[2] << endl;
		for (int b = 0; b < FFD_lattice.size(); b++){


			int i  = b / (lattice_ny*lattice_nz);
			int j = (b % (lattice_ny*lattice_nz)) / lattice_nz;
			int k = (b % (lattice_ny*lattice_nz)) % lattice_nz;
			//int i = b % lattice_nx;
			//int j = b / (lattice_nx*lattice_nz);
			//int k = b % lattice_nz;
			//cout << i << ", " << j << ", " << k << "\n";

			//ex: is b is 18, need to use b to find vertex 18 in the lattice 

			float tx = (m.vertices[a].p[0] - BoundingBox[0]) / (BoundingBox[1] - BoundingBox[0]);			//is this right?
			float ty = (m.vertices[a].p[1] - BoundingBox[2]) / (BoundingBox[3] - BoundingBox[2]);
			float tz = (m.vertices[a].p[2] - BoundingBox[4]) / (BoundingBox[5] - BoundingBox[4]);

			float bcx=factorial(lattice_nx - 1) / (factorial(i)*factorial((lattice_nx-1) - i));	//binomial coefficient for x		d=lattice_nx-1
			float bcy=factorial(lattice_ny - 1) / (factorial(j)*factorial((lattice_ny-1) - j));	//binomial coefficient for y
			float bcz=factorial(lattice_nz - 1) / (factorial(k)*factorial((lattice_nz-1) - k));	//binomial coefficient for z
			//cout << bcx << ", " << bcy << ", " << bcz << "\n";

			//TODO: convert v.p (the position of p) into the parameterized space using FFD_lattice
			//vector<double> weights;
			float weightx = bcx*pow(tx, i)*pow(1 - tx, (lattice_nx - 1) - i);
			float weighty = bcy*pow(ty, j)*pow(1 - ty, (lattice_ny - 1) - j);
			float weightz = bcz*pow(tz, k)*pow(1 - tz, (lattice_nz - 1) - k);
			float weights = weightx*weighty*weightz;
			//FFD_parameterization[0][0];
			FFD_param[a][b]=weights; //=calculated weight
