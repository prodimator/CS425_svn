...
void put_dof_rot(string const &dof, Quaternion const &rot, double *info, int start_pos);
...
Quaternion ordered_rotation(string const &order, Vector3d const &rotation);
...
namespace {
Quaternion ordered_rotation(string const &order, Vector3d const &rot) {
	Quaternion ret;
	//ret.clear();
	...
	for (unsigned int i = 0; i < order.size(); ++i) {
		switch (order[i]) {
			case 'x':
				ret = Quaternion::get(rot[0] * M_PI / 180.0, Vector3d(1.0, 0.0, 0.0))*ret;
				break;
			case 'y':
				ret = Quaternion::get(rot[1] * M_PI / 180.0, Vector3d(0.0, 1.0, 0.0))*ret;
				break;
			case 'z':
				ret = Quaternion::get(rot[2] * M_PI / 180.0, Vector3d(0.0, 0.0, 1.0))*ret;
				break;
			case 'X':
			case 'Y':
			case 'Z':
...
Quaternion get_dof_rot(string const &dof, double const *info, int start_pos) {
	...
	for (unsigned int i = 0; i != dof.size(); ++i) {
		...
		switch (dof[i]) {
			case 'x':
				ret = Quaternion::get(d * M_PI / 180.0, Vector3d(1.0, 0.0, 0.0)) * ret;
				break;
			case 'y':
				ret = Quaternion::get(d * M_PI / 180.0, Vector3d(0.0, 1.0, 0.0)) * ret;
				break;
			case 'z':
				ret = Quaternion::get(d * M_PI / 180.0, Vector3d(0.0, 0.0, 1.0)) * ret;
				break;
			case 'a':
				{
					Vector3d axis = *(Vector3d*)info;
					info += 2;
					ret = Quaternion::get(axis.norm(), axis.normalize()) * ret;
					break;
				}
...
namespace 
{
	...
	double get_rotation(Vector3d axis, Vector3d probe, Quaternion const &rot) 
	{
		Vector3d perp = axis%probe;
		Vector3d rotated = rot.rotate(probe); //(rot*probe*(-rot)).getComplex(); //rotate(probe, rot);
		rotated = (rotated - axis * (axis * rotated)).normalize();
...
void put_dof_rot(string const &dof, Quaternion const &rot, double *info, int start_pos) {
	...
	for (unsigned int i = 0; i != dof.size(); ++i) {
		switch (dof[i]) {
			case 'x':
				vec[count] = Vector3d(1.0, 0.0, 0.0);
				perp[count] = Vector3d(0.0, 1.0, 0.0);
				ind[count] = i;
				//q[count+1] = rot.x;
				++count;
				break;
			case 'y':
				vec[count] = Vector3d(0.0, 1.0, 0.0);
				perp[count] = Vector3d(0.0, 0.0, 1.0);
				ind[count] = i;
				//q[count+1] = rot.y;
				++count;
				break;
			case 'z':
				vec[count] = Vector3d(0.0, 0.0, 1.0);
				perp[count] = Vector3d(1.0, 0.0, 0.0);
				ind[count] = i;
				//q[count+1] = rot.z;
				++count;
				break;
			case 'a':
				//Something special here, I guess.
				{
					...
					double xyzl = rot.getComplex().norm(); //length(rot.xyz);
					double theta;
					if (xyzl != 0.0 && rot.getReal() != 0) {
						theta = 2 * atan(xyzl / rot.getReal() );
						vec = rot.getComplex() * theta / xyzl;
					} else {
						vec = rot.getComplex();
					}
					return;
	...
	if (count == 0) {
		...
		info[start_pos + ind[0]] = 180.0 / M_PI * get_rotation(vec[0], perp[0], (-(Quaternion::get(ang1, vec[1]))*rot));
	} else if (count == 3) {
		//a 3-d rotation == "problem case"
		// rot is a quaternion
		// vec[0], vec[1], vec[2] are axes, orthonormal
		// create Euler angles in info around these axes
		Vector3d new0 = rot.rotate(vec[0]);
		Vector3d new1 = rot.rotate(vec[1]);
		Vector3d new2 = rot.rotate(vec[2]);
...
void Skeleton::build_pose(double const *frame_data, Character::Pose &pose) const {
	...
	pose.root_orientation = (ordered_rotation(offset_order, axis_offset) * get_dof_rot(order, frame_data, 0));

	for (unsigned int b = 0; b < bones.size(); ++b) {
		Quaternion rot;
		//rot.clear();
		rot = (-(bones[b].global_to_local)) * rot;
		Quaternion mul = get_dof_rot(bones[b].dof, frame_data, bones[b].frame_offset);
		rot = (mul*rot);
		rot = (bones[b].global_to_local*rot);
		rot = rot.normalize();
	...
	if (rot_is_glob) {
		for (unsigned int b = bones.size()-1; b < bones.size(); --b) { //neg wraps
			if (bones[b].parent==-1) {
				...
				pose.bone_orientations[b] = (-(f)) * pose.bone_orientations[b];
			} else {
				pose.bone_orientations[b] = (-(pose.bone_orientations[bones[b].parent])) * pose.bone_orientations[b];
			}
			pose.bone_orientations[b] = pose.bone_orientations[b].normalize();
	...
	if (z_is_up) {
		Quaternion rot;
		rot = Quaternion::get(-(float)M_PI * 0.5f, Vector3d(1.0f, 0.0f, 0.0f));
		pose.root_position = rot.rotate(pose.root_position);
...
void Skeleton::get_angles(Character::Pose const &from, double *to) const {
	...
	put_dof_trans(order, from.root_position - position, to, 0);
	...
	root_orientation = (-(ordered_rotation(offset_order, axis_offset)))* root_orientation;
	...
	for (unsigned int b = 0; b < bones.size(); ++b) {
		...
		rot = (rot * bones[b].global_to_local);
		rot = (-(bones[b].global_to_local)) * rot;
		put_dof_rot(bones[b].dof, rot, to, bones[b].frame_offset);
		//DEBUG:
		Quaternion other = get_dof_rot(bones[b].dof, to, bones[b].frame_offset);
		if ((other - rot).norm() > 0.01
		 && ((-other) - rot).norm() > 0.01) {
			cout << "Conversion of " << rot << " fails: " << other << "bone: " << bones[b].name << endl;
			cout << " Residual: " << ((-other)* rot) << endl;
