...
void get_world_bones(Pose const &pose, WorldBones &out) {
	...
	vector< Quaternion > orientation_stack;
	...
	for (unsigned int b = 0; b < pose.bone_orientations.size(); ++b) {
		...
		orientation_stack.push_back( (orientation_stack.back() * pose.bone_orientations[b]).normalize() );
		...
		position_stack.push_back(position_stack.back() + orientation_stack.back().rotate( (pose.skeleton->bones[b].direction * pose.skeleton->bones[b].length) ) );
