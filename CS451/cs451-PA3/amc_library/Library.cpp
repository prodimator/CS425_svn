...
Vector3d AnnotationColors[AnnotationCount] = {
	Vector3d( 0.5f, 0.7f, 0.5f ),
	Vector3d( 0.7f, 0.5f, 0.5f ),
	Vector3d( 0.2f, 0.2f, 1.0f),
	Vector3d( 0.5f, 0.5f, 1.0f),
	Vector3d( 0.1f, 0.5f, 1.0f),
...
namespace {
	...
	inline Vector3d rotate_by_yaw(Vector3d const &in, float const yaw) {
		float c = cosf(yaw);
		float s = sinf(yaw);
		return Vector3d(s * in[2] + c * in[0], in[1], c * in[2] - s * in[0]);
	}

	inline 
	double get_yaw_angle(Quaternion const &axis) {
		Vector3d test = axis.rotate(Vector3d(1.0f, 0.0f, 0.0f));
		return atan2(-test[2], test[0]);
	}
}
...
void Motion::get_local_root(unsigned int frame, Vector3d &position, Quaternion &orientation) const {
	assert(loaded);
...
void Motion::calculate_control_data() 
{
	...
	for (unsigned int i = 0; i < frames(); ++i) {
		...
		smooth_root[i].position = Vector3d(0.0f, 0.0f, 0.0f);
		float mass = 0.0f;
		for (unsigned int b = 0; b < wb.bases.size(); ++b) {
			...
			smooth_root[i].position = smooth_root[i].position+ 0.5f * m * (wb.bases[b] + wb.tips[b]);
			mass += m;
		}
		if (mass == 0.0f) {
			cout << "Zero mass determining smooth root." << endl;
		} else {
			smooth_root[i].position = smooth_root[i].position/mass;
		}
		smooth_root[i].position[1] = 0.0f; //project.
	...
	for (unsigned int i = 0; i < frames(); ++i) 
	{
		Character::Pose pose;
		get_pose(i, pose);
		Quaternion inv = -Quaternion::get(smooth_root[i].orientation, Vector3d(0.0f, 1.0f, 0.0f));
		local_root[i].position = inv.rotate(pose.root_position - smooth_root[i].position);
		local_root[i].orientation = (inv * pose.root_orientation);
		...
		for (unsigned int b = 0; b < world_bones.tips.size(); ++b) {
			if (b == 0) distance_to_floor[i] = world_bones.tips[b][1];
			if (world_bones.tips[b][1] < distance_to_floor[i])
				distance_to_floor[i] = world_bones.tips[b][1];
			if (world_bones.bases[b][1] < distance_to_floor[i])
				distance_to_floor[i] = world_bones.bases[b][1];
		}
	}
	...
	for (unsigned int i = 0; i + 1 < frames(); ++i) {
		...
		Vector3d position_delta = smooth_root[i+1].position - smooth_root[i].position;
		Quaternion q = -Quaternion::get(smooth_root[i].orientation, Vector3d(0.0f, 1.0f, 0.0f));
		control_data[i].desired_velocity = q.rotate(smooth_root[i+1].position - smooth_root[i].position) * inv_ts;
