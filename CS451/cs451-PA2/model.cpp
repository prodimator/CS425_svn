...
bool model::build(const string & name)
{
    ...
    {//build/copy model
        ...
        for(uint i=0;i<v_size;i++){
			...
			//find min positions and max positions
			if (vertices[i].p[0] < minX){ minX = vertices[i].p[0]; }
			if (vertices[i].p[0] > maxX){ maxX = vertices[i].p[0]; }
			if (vertices[i].p[1] < minY){ minY = vertices[i].p[1]; }
			if (vertices[i].p[1] > maxY){ maxY = vertices[i].p[1]; }
			if (vertices[i].p[2] < minZ){ minZ = vertices[i].p[2]; }
			if (vertices[i].p[2] > maxZ){ maxZ = vertices[i].p[2]; }

			
		}
		//cout << "minX: " << minX<< " " << "maxX: " << maxX << endl;			//debug-- prints out mins and maxs verts for bounding box
