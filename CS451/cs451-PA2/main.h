...
vector<Point3d> FFD_parameterization; //This stores all parameterized coordinates of all vertices from the model
                                      //FFD_parameterization has size = models.front().v_size			number of values is nx*ny*nz*number of verts in entire model
...
void computeCOM_R()
{
	...
	float incZ = (box[5]-box[4]) / (lattice_nz - 1);
	float incY = (box[3]-box[2]) / (lattice_ny - 1);
	float incX = (box[1]-box[0]) / (lattice_nx - 1);

	for (int k = 0; k < lattice_ny; k++){
		for (int j = 0; j < lattice_nx; j++){
			for (int i = 0; i < lattice_nz; i++){
				Point3d pt;
				pt[0] = tminX;
				pt[1] = tminY;
				pt[2] = tminZ;
				FFD_lattice.push_back(pt);
				tminZ += incZ;
			}
			tminZ = box[4];
			tminX += incX;
